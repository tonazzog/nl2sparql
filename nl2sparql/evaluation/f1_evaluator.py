"""F1 Score on Answers evaluator for NL2SPARQL system.
...
Usage:
    from nl2sparql.evaluation.f1_evaluator import F1Evaluator
...
def build_variable_mapping(
    ...
    # Phase 1: Match by category and position
    for category in ["primary", "secondary", "aggregates", "numeric"]:
        ...
        for i, gold_var in enumerate(gold_vars):
            if i < len(pred_vars) and pred_vars[i] not in used_predicted:
                mapping[gold_var] = pred_vars[i]
                used_predicted.add(pred_vars[i])
...
class F1Evaluator:
    ...
    def __init__(
        ...
        strip_predicted_limit: bool = False,
    ):
        self.endpoint = endpoint
        self.timeout = timeout
        self.max_results = max_results
        self.cache_gold_results = cache_gold_results
        self.strip_predicted_limit = strip_predicted_limit
    ...
    def evaluate_dataset(
        ...
        for tc in test_cases:
            ...
            stored_sparql = strip_limit_offset(predicted_sparql) if self.strip_predicted_limit else predicted_sparql
